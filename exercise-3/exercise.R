...
US.Personal.Expenditure <- data.frame(USPersonalExpenditure, stringsAsFactor = FALSE)
...
# The row names have a 'x' in front of the dates
...
US.Personal.Expenditure$X1960[max(US.Personal.Expenditure$X1960) == US.Personal.Expenditure$X1960]

# Define a function `DetectHighest` that takes in a year as a parameter, and
# returns the highest spending category of that year

DetectHighest <- function(year) {
  highest <- US.Personal.Expenditure$xyear[max(US.Personal.Expenditure$xyear) == US.Personal.Expenditure$xyear]
  return(highest)
}
